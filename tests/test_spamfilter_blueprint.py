...
class TestIndexPage(BaseTestCase):
    ...
    def test_get_response(self):
        response = self.client.get('/')
        self.assertIn(
            b'Welcome to SPAM FILTER application. This application can be used to perform the following tasks.',
            response.data)
...
class TestUploadPage(BaseTestCase):
    ...
    def test_get_response(self):
        ...
        self.assertIn(b'<input type = "file" name = "file" />', response.data)
    ...
    def test_post_url_response1(self):
        response = self.client.post('/upload/',
                                    data=dict(
                                        file=(os.path.join(current_app.config['TEST_DATA_DIR'], 'sample_emails.csv'),
                                              'sample_emails.csv')
                                    ),
        ...
        self.assertIn(b'File : <span style="color:green">sample_emails.csv</span> is uploaded successfully',
                      response.data)
    ...
    def test_post_url_response3(self):
        response = self.client.post('/upload/',
                                    data=dict(
                                        file=(os.path.join(current_app.config['TEST_DATA_DIR'], 'sample_email3.txt'),
                                              'sample_email3.txt')
                                    ),
                                    content_type='multipart/form-data',
                                    follow_redirects=True)
...
class TestTrainPage(BaseTestCase):
    ...
    def test_post_response1(self):
        ...
        self.assertIn(b'Model : <span style="color:green">sample_emails.pk</span> is successfully created.',
                      response.data)
        self.assertTrue(os.path.exists(os.path.join(current_app.config['ML_MODEL_UPLOAD_FOLDER'], 'sample_emails.pk')))
        self.assertTrue(os.path.exists(
            os.path.join(current_app.config['ML_MODEL_UPLOAD_FOLDER'], 'sample_emails_word_features.pk')))
    ...
    def test_post_response2(self):
        ...
        def test_post_response6(self):
            ...
            self.assertEqual(3, response.data.count(
                b'<span style="color:red;font-size:20px;font-weight:bold;">SPAM</span>'))